...
export default function SolicitarEnvio() {
  ...
  const [origenGeo, setOrigenGeo] = useState({
    placeId: "",
    lat: null,
    lng: null,
    texto: "",
  ...
  const [destinoGeo, setDestinoGeo] = useState({
    placeId: "",
    lat: null,
    lng: null,
    texto: "",
  ...
  // Helpers GEO
  const provinciaNombreById = (id) =>
    provincias.find((p) => p.id === id)?.nombre || "";
  const localidadNombreById = (list, id) =>
    list.find((l) => l.id === id)?.nombre || "";
  ...
  async function refreshDestinatarios() {
    ...
    const dests = Array.isArray(raw)
      ? raw
      : raw?.destinatarios || raw?.data || [];
  ...
  async function loadLists() {
    try {
      ...
      setError(
        getApiErrorMessage(
          e,
          "No se pudieron cargar direcciones/destinatarios."
        )
      );
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      try {
        setOrigenSug(await getAddressSuggestions(origenQuery));
      } catch {
        setOrigenSug([]);
      }
    }, 350);
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      try {
        setDestinoSug(await getAddressSuggestions(destinoQuery));
      } catch {
        setDestinoSug([]);
      }
    }, 350);
  ...
  async function loadLocalidades(setter, provId) {
    ...
    if (!provId) return;
    try {
      setter(await getLocalidadesByProvincia(provId));
    } catch (e) {
    ...
    if (name === "origen") {
      ...
      if (value !== "otra") {
        setForm((p) => ({
          ...
          origenProvincia: "",
          origenCP: "",
          origenReferencia: opt?.referencia || "",
        }));
  ...
  function onChangePaquete(index, e) {
    ...
    setForm((p) => ({
      ...p,
      paquetes: p.paquetes.map((paq, i) =>
        i === index ? { ...paq, [name]: value } : paq
      ),
    }));
  }
  ...
  function getOrigenData() {
    if (form.origen === "otra") {
      return {
        ...
        referencia: form.origenReferencia.trim(),
      };
    }
    const opt = direccionesRetiro.find((d) => d.value === form.origen);
    return {
      direccion: opt?.direccion || "",
      localidad: opt?.localidadNombre || "",
      provincia: opt?.provinciaNombre || "",
      cp: opt?.cp || "",
  ...
  function getDestinoTexto() {
    if (form.destinatarioId && form.destinatarioId !== "nuevo") {
      ...
      if (sel) {
        return [sel.direccion, sel.localidadNombre, sel.provinciaNombre]
          .filter(Boolean)
          .join(", ");
      }
      return "";
  ...
  function validateDestino() {
    ...
    if (!form.ciudad) return "Seleccioná la ciudad.";
    ...
    if (!/^[\d+\s()-]{6,20}$/.test(form.telefono.trim()))
  ...
  function validateBeforeSubmit() {
    ...
    if (form.origen === "otra") {
      ...
      if (!form.origenLocalidad) return "Seleccioná la ciudad de retiro.";
      ...
      const o = getOrigenData();
      if (!o.localidad) {
        return "La dirección de retiro seleccionada no tiene localidad.";
      }
    }

    if (!form.franjaHorariaRetiro) {
      return "Seleccioná una franja horaria de disponibilidad para el retiro.";
    }
    ...
    for (const [i, p] of form.paquetes.entries()) {
      if (!p.tipoPaquete) {
        return `Seleccioná el tipo de paquete (Paquete #${i + 1}).`;
      }
      if (!p.alto || !p.ancho || !p.profundidad) {
        return `Completá dimensiones (Paquete #${i + 1}).`;
      }
      if (!p.peso) return `Completá el peso (Paquete #${i + 1}).`;
      if (!p.cantidad || parseInt(p.cantidad) < 1) {
        return `La cantidad debe ser al menos 1 (Paquete #${i + 1}).`;
      }
    }
  ...
  function buildCrearEnvioPayloadBase() {
    const origenLocalidadId =
      form.origen === "otra"
        ...
        : direccionesRetiro.find((d) => d.value === form.origen)?.localidadId || "";

    const origenLocalidadNombre =
      form.origen === "otra"
        ? localidadNombreById(localidadesOrigen, form.origenLocalidad)
        : direccionesRetiro.find((d) => d.value === form.origen)?.localidadNombre || "";
    ...
    const destinoLocalidadNombre =
      form.destinatarioId && form.destinatarioId !== "nuevo"
        ? clientsGuardadosSafeFind(clientesGuardados, form.destinatarioId)?.localidadNombre || ""
  ...
  async function goBuscar() {
    ...
    try {
      ...
      const bultos = form.paquetes.reduce(
        (acc, p) => acc + Math.max(1, parseInt(p.cantidad) || 1),
        0
      );
  ...
  function openModalGuardarOrigen() {
    if (!form.origenDireccion.trim()) {
      return setError("Completá la dirección de retiro antes de guardar.");
    }
    if (!form.origenProvincia) {
      return setError("Seleccioná la provincia de retiro antes de guardar.");
    }
    if (!form.origenLocalidad) {
      return setError("Seleccioná la ciudad de retiro antes de guardar.");
    }
    if (!form.origenCP.trim()) {
      return setError("Completá el código postal de retiro antes de guardar.");
    }
    if (origenGeo.lat == null || origenGeo.lng == null) {
      return setError(
        "Elegí una sugerencia de dirección (Google) para guardar las coordenadas."
      );
    }
    ...
    setOrigenModalForm({
      ...
      cp: form.origenCP,
      referencia: form.origenReferencia,
    });
  ...
  async function guardarNuevaDireccionOrigen() {
    ...
    const locName = localidadNombreById(
      localidadesOrigenModal,
      origenModalForm.ciudad
    );
    ...
    const f = {
      ...
      referencia: origenModalForm.referencia.trim(),
    ...
    if (!f.provincia.provinciaId || !f.localidad.localidadId) {
      setOrigenModalError("Seleccioná provincia y ciudad.");
      return;
    }
    if (!f.localidad.localidadNombre || !f.provincia.provinciaNombre) {
      setOrigenModalError(
        "No se pudo resolver el nombre de provincia/ciudad."
      );
      return;
    }
    ...
    try {
      ...
      setOrigenModalError(
        getApiErrorMessage(e, "No se pudo guardar la dirección.")
      );
    } finally {
      setLoading(false);
    }
  ...
  function openModalGuardarDestino() {
    ...
    if (destinoGeo.lat == null || destinoGeo.lng == null) {
      setError(
        "Elegí una sugerencia de dirección (Google) para confirmar la ubicación del DESTINO."
      );
      return;
    }
  ...
  async function guardarNuevoClienteDestino() {
    ...
    const locName = localidadNombreById(
      localidadesDestinoModal,
      destinoModalForm.ciudad
    );
    ...
    const f = {
      ...
      referencia: destinoModalForm.referencia.trim(),
    ...
    if (!f.apellido || !f.nombre) {
      return setDestinoModalError("Completá apellido y nombre.");
    }
    if (!f.dni || !/^\d{7,8}$/.test(f.dni)) {
      return setDestinoModalError("DNI inválido (7 u 8 dígitos).");
    }
    ...
    if (!f.provincia.provinciaId) {
      return setDestinoModalError("Seleccioná la provincia.");
    }
    if (!f.localidad.localidadId) {
      return setDestinoModalError("Seleccioná la ciudad.");
    }
    if (!f.cp) return setDestinoModalError("Completá el código postal.");
    if (!f.placeId) {
      return setDestinoModalError(
        "Elegí una dirección de Google antes de guardar."
      );
    }
    ...
    try {
      ...
      if (newId) {
        setForm((p) => ({
          ...
          provincia:
            created?.provincia?.provinciaId ?? f.provincia.provinciaId,
          ciudad: created?.localidad?.localidadId ?? f.localidad.localidadId,
          cp: created.cp ?? f.cp,
          referencia: created.referencia ?? f.referencia,
        }));
      ...
      setDestinoModalError(
        getApiErrorMessage(e, "No se pudo guardar el destinatario.")
      );
    } finally {
      setLoading(false);
    }
    ...
    <main className="bg-slate-100">
              ...
              <Section title="Origen" icon={MapPin}>
                <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                  <Select
                    ...
                    value={form.origen}
                    onChange={onChange}
                    options={direccionesRetiro}
                    placeholder="Dirección de retiro"
                  />

                  <Select
                    name="franjaHorariaRetiro"
                    value={form.franjaHorariaRetiro}
                    onChange={onChange}
                    options={FRANJAS_HORARIAS}
                    placeholder="¿En qué horario estás disponible para el retiro?"
                  />
                  ...
                  {form.origen === "otra" && (
                    <div className="md:col-span-2 grid grid-cols-12 gap-2">
                      ...
                      <Select
                        ...
                        value={form.origenLocalidad}
                        onChange={onChange}
                        options={localidadesOrigen.map((l) => ({
                          value: l.id,
                          label: l.nombre,
                        }))}
                        placeholder="Seleccionar ciudad"
                        disabled={!form.origenProvincia}
                      />

                      <input
                        className="col-span-4 w-full rounded-md border px-4 py-2 outline-none text-slate-700"
                        name="origenCP"
                        value={form.origenCP}
                        onChange={onChange}
                      ...
                      <textarea
                        className="col-span-12 min-h-[96px] w-full rounded-md border px-4 py-3 outline-none text-slate-700"
                      ...
                      <button
                        className="mt-2 font-semibold text-blue-700 hover:underline col-span-12 text-left"
                        type="button"
                        onClick={openModalGuardarOrigen}
                      >
              ...
              <Section title="Destino" icon={MapPin}>
                <div className="grid grid-cols-1 gap-2 md:grid-cols-12">
                  <Select
                    ...
                    onChange={onChange}
                    options={clientesGuardados.map((c) => ({
                      value: c.id,
                      label: c.label,
                    }))}
                  ...
                  {form.destinatarioId === "nuevo" && (
                    <div className="md:col-span-12 grid grid-cols-12 gap-2">
                      <input
                        className="col-span-6 w-full rounded-md border px-4 py-2 outline-none text-slate-700"
                        name="apellido"
                        value={form.apellido}
                        onChange={onChange}
                      ...
                      />
                      <input
                        className="col-span-3 w-full rounded-md border px-4 py-2 outline-none text-slate-700"
                        name="dni"
                        value={form.dni}
                        onChange={onChange}
                        placeholder="DNI"
                      ...
                      <Select
                        ...
                        value={form.ciudad}
                        onChange={onChange}
                        options={localidadesDestino.map((l) => ({
                          value: l.id,
                          label: l.nombre,
                        }))}
                        placeholder="Seleccionar ciudad"
                      ...
                      <button
                        className="mt-2 font-semibold text-blue-700 hover:underline col-span-12 text-left"
                        type="button"
                        onClick={openModalGuardarDestino}
                      >
                  ...
                  <div className="flex items-center gap-2 text-md font-bold text-slate-700 col-span-12 mt-2">
                    ...
                    <DatePicker
                      ...
                      locale="es"
                      calendarStartDay={1}
                      minDate={fromISODate(getTodayString())}
                    />
                  </div>
                </div>
              </Section>
                  ...
                  {form.paquetes.map((paq, index) => (
                    <div key={index} className="grid grid-cols-1 md:grid-cols-12">
                      <div className="grid grid-cols-1 md:grid-cols-12 col-span-11 gap-2">
                        <input
                          className="col-span-1 w-full rounded-md border px-4 py-2 outline-none text-slate-700"
                          name="cantidad"
                          value={paq.cantidad}
                          onChange={(e) => onChangePaquete(index, e)}
                          placeholder="Cant."
                          type="number"
                          min="1"
                        />
                        ...
                        {["alto", "ancho", "profundidad"].map((f) => (
                          <input
                            key={f}
                            className="col-span-2 w-full rounded-md border px-4 py-2 outline-none text-slate-700"
                            name={f}
                            value={paq[f]}
                            onChange={(e) => onChangePaquete(index, e)}
      ...
      {openOrigenModal && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center">
          <button
            type="button"
            onClick={() => setOpenOrigenModal(false)}
              ...
              {origenModalStep === "form" ? (
                <>
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <Input
                      name="alias"
                      value={origenModalForm.alias}
                      onChange={onChangeOrigenModal}
                      placeholder="Alias (Casa, Trabajo...)"
                    />
                    <Input
                      name="direccion"
                      value={origenModalForm.direccion}
                      onChange={onChangeOrigenModal}
                      placeholder="Dirección"
                    />
                    <Select
                      name="provincia"
                      value={origenModalForm.provincia}
                      onChange={onChangeOrigenModal}
                      options={provincias.map((p) => ({
                        value: p.id,
                        label: p.nombre,
                    ...
                    <Select
                      name="ciudad"
                      value={origenModalForm.ciudad}
                      onChange={onChangeOrigenModal}
                      options={localidadesOrigenModal.map((l) => ({
                        value: l.id,
                        label: l.nombre,
                      }))}
                      placeholder="Seleccionar ciudad"
                      disabled={!origenModalForm.provincia}
                    />
                    <Input
                      name="cp"
                      value={origenModalForm.cp}
                      onChange={onChangeOrigenModal}
                      placeholder="Código postal"
                    ...
                    <div className="md:col-span-2">
                      <textarea
                        name="referencia"
                        value={origenModalForm.referencia}
                        onChange={onChangeOrigenModal}
                        placeholder="Referencia (opcional)"
                        className="min-h-[96px] w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                      />
                    </div>
                  </div>
                  ...
                  <div className="mt-6 flex justify-end gap-2">
                    ...
                    <Button
                      onClick={guardarNuevaDireccionOrigen}
                      disabled={loading}
                    >
                ...
                <div className="py-6 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-slate-600">
                      Se agregó{" "}
                      <span className="font-semibold">{lastSavedAlias}</span> a
                      tus direcciones.
                    </div>
                  </div>
                  <div className="ml-auto">
                    <Button onClick={() => setOpenOrigenModal(false)}>
                      Listo
                    </Button>
                  </div>
                </div>
      ...
      {openDestinoModal && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center">
          <button
            type="button"
            onClick={() => setOpenDestinoModal(false)}
            className="absolute inset-0 bg-black/40"
            aria-label="Cerrar"
          />
          <div className="relative z-[1000] w-[92vw] max-w-2xl">
              ...
              {destinoModalStep === "form" ? (
                <>
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <Input
                      name="apellido"
                      value={destinoModalForm.apellido}
                      onChange={onChangeDestinoModal}
                      placeholder="Apellido"
                    />
                    <Input
                      name="nombre"
                      value={destinoModalForm.nombre}
                      onChange={onChangeDestinoModal}
                      placeholder="Nombre"
                    />
                    <Input
                      name="dni"
                      value={destinoModalForm.dni}
                      onChange={onChangeDestinoModal}
                      placeholder="DNI"
                    />
                    <Input
                      name="telefono"
                      value={destinoModalForm.telefono}
                      onChange={onChangeDestinoModal}
                      placeholder="Teléfono"
                    />
                    <Input
                      name="direccion"
                      value={destinoModalForm.direccion}
                      onChange={onChangeDestinoModal}
                      placeholder="Dirección"
                      className="md:col-span-2"
                    />
                    <Select
                      name="provincia"
                      value={destinoModalForm.provincia}
                      onChange={onChangeDestinoModal}
                      options={provincias.map((p) => ({
                        value: p.id,
                        label: p.nombre,
                    ...
                    <Select
                      name="ciudad"
                      value={destinoModalForm.ciudad}
                      onChange={onChangeDestinoModal}
                      options={localidadesDestinoModal.map((l) => ({
                        value: l.id,
                        label: l.nombre,
                      }))}
                      placeholder="Seleccionar ciudad"
                      disabled={!destinoModalForm.provincia}
                    />
                    <Input
                      name="cp"
                      value={destinoModalForm.cp}
                      onChange={onChangeDestinoModal}
                      placeholder="Código postal"
                      className="md:col-span-2"
                    />
                    <div className="md:col-span-2">
                      <textarea
                        name="referencia"
                        value={destinoModalForm.referencia}
                        onChange={onChangeDestinoModal}
                        placeholder="Referencia (opcional)"
                        className="min-h-[96px] w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                      />
                  ...
                  <div className="mt-6 flex justify-end gap-2">
                    ...
                    <Button
                      onClick={guardarNuevoClienteDestino}
                      disabled={loading}
                    >
                      {loading ? "Guardando..." : "Guardar"}
                    </Button>
                  ...
                  <div>
                    ...
                    <div className="text-slate-600">
                      ...
                      <span className="font-semibold">
                        {lastSavedClienteNombre}
                      </span>{" "}
                      a tus destinatarios.
                    </div>
                  </div>
                  <div className="ml-auto">
                    <Button onClick={() => setOpenDestinoModal(false)}>
                      Listo
                    </Button>
                  </div>
                </div>
...
function Select({
  ...
  return (
    <select
      {...props}
      className={`w-full rounded-lg border px-4 py-2 outline-none text-slate-700 bg-white disabled:bg-slate-100 disabled:text-slate-400 ${className}`}
    >
...
function clientsGuardadosSafeFind(clientesGuardados, destinatarioId) {
  return clientesGuardados.find((c) => c.id === destinatarioId);
}