...
export default function DashboardComisionista() {
  ...
  // Ref que siempre tiene el valor actual — seguro de leer dentro de callbacks async
  ...
  const cargarOGenerarRuta = useCallback(async (forceRegenerate = false, coordenadas = null, syncViaje = false) => {
    ...
    try {
      ...
      // forceRegenerate: regenerar con la posición actual
  ...
  // Solo recarga el objeto ruta — nunca toca viajeIniciado
  ...
  }, [cargarAgenda, cargarEstadisticas, toast]);
    ...
    toast.confirm("¿Finalizar el viaje del día? Los envíos pendientes quedarán como demorados.", {
      ...
      onConfirm: async () => {
        await withLoading("finalizar_viaje", async () => {
          ...
          setRuta(null);
          // Recargar agenda ANTES de decidir si hay demorados pendientes
          const [r1, r2] = await Promise.all([
            getDashboardResumen({ date: fechaHoy }),
            getAgendaHoy({ date: fechaHoy }),
          ]);
          setResumen(r1);
          const nuevaAgenda = Array.isArray(r2?.items ?? r2) ? (r2?.items ?? r2) : [];
          setAgenda(nuevaAgenda);
          // Hay demorados pendientes → viaje no termina, hay que iniciar otro tramo
          const hayDemorados = nuevaAgenda.some(e =>
            ["DEMORADO_RETIRO", "DEMORADO_ENTREGA"].includes(e.estado)
          );
          // Solo limpiar viajeIniciado si no hay pendientes de ningún tipo
          const hayPendientes = nuevaAgenda.some(e =>
            !["ENTREGADO", "CANCELADO", "DEVUELTO", "CANCELADO_RETORNO"].includes(e.estado)
          );
          setViajeIniciadoPersist(false);
          if (hayDemorados) {
  ...
  const stats = useMemo(() => {
    ...
    const ingresoPromedioViaje   = Number(estadisticas?.ingresoPromedio || estadisticas?.ingresoPromedioViaje || 0);
    const distanciaPromedioViaje = Number(estadisticas?.distanciaPromedio || estadisticas?.distanciaPromedioViaje || 0);
    const diasMasEntregas =
      Array.isArray(estadisticas?.diasMasEntregas) && estadisticas.diasMasEntregas.length > 0
        ? estadisticas.diasMasEntregas
        : ["Lun","Mar","Mié","Jue","Vie","Sáb","Dom"].map(dia => ({ dia, entregas: 0 }));
    const comparativaEnvios =
      Array.isArray(estadisticas?.comparativaEnvios) && estadisticas.comparativaEnvios.length > 0
    ...
    return { ingresosTotales, ingresoPromedioViaje, distanciaPromedioViaje, diasMasEntregas, comparativaEnvios };
  }, [estadisticas]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        {/* Botón viaje: Finalizar siempre visible si el viaje está activo,
            Iniciar solo si hay envíos pendientes y el viaje no está activo */}
        {viajeIniciado ? (
          <button
            type="button"
      ...
      <div className="space-y-4">
        <h2 className="text-2xl font-bold text-blue-800">Estadísticas de actividad</h2>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <StatCard
          ...
          <StatCard
            ...
            value={`$${stats.ingresoPromedioViaje.toLocaleString("es-AR", { maximumFractionDigits: 0 })}`}
          />
          <StatCard
            icon={<Map className="h-5 w-5" />}
            label="Distancia promedio por viaje"
            value={`${stats.distanciaPromedioViaje.toLocaleString("es-AR", { maximumFractionDigits: 1 })} km`}
          />
        </div>
