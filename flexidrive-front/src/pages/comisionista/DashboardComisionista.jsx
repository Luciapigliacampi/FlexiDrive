          ...
          if (e?.response?.status === 404) {
            ...
            if (syncViaje) {
              try {
                ...
                const hayEnRuta = items.some(e =>
                  ['EN_CAMINO', 'DEMORADO_ENTREGA', 'EN_RETIRO', 'RETIRADO'].includes(e.estado)
                );
...
function AgendaRow({ row, actionLoading, onRetirar, onEntregar }) {
  ...
  const soloDir = row.direccion
    ? row.direccion
    : (row.destino ?? "").split(",")[0].trim();
    ...
    <tr className="border-t border-slate-100 text-xs hover:bg-slate-50">
      ...
      <td className="px-3 py-3">
        <div className="flex flex-col gap-0.5">
          <Link to={`/comisionista/envios/${row.id}/tracking`} className="font-bold text-blue-700 hover:underline">
            #{row.numero}
          </Link>
          <span className={`inline-block font-bold uppercase px-1.5 py-0.5 rounded-full w-fit ${
      ...
      <td className="px-3 py-3 text-slate-700 max-w-[160px]">
        ...
        {row.franja && <span className="text-slate-400 block mt-0.5">🕐 {row.franja}</span>}
      </td>
      <td className="px-3 py-3 text-slate-700">{row.localidad}</td>
      <td className="px-3 py-3">
...
function ActionBtn({ title, loading, onClick, colorCls, icon, label }) {
  return (
    <button
      ...
      onClick={onClick}
      disabled={loading}
      className={`flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-bold text-white ${colorCls} disabled:opacity-40 transition`}
    >
      {loading
