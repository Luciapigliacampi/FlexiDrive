...
function pasoActual(estadoId) {
  ...
  // Si está demorado, visualmente lo mostramos en el último punto operativo alcanzado
 if (estadoId === "DEMORADO_RETIRO") {
    return ORDEN_ESTADOS.indexOf("EN_RETIRO");
  }
  if (estadoId === "DEMORADO_ENTREGA") {
    return ORDEN_ESTADOS.indexOf("EN_CAMINO");
  }
...
export default function TrackingEnvioComisionista() {
  ...
  useEffect(() => {
    ...
    api
      .get(`${CAL_BASE}/api/calificaciones/envio/${id}`)
 ...
 const puedeCancelar =
    ["PENDIENTE", "ASIGNADO", "EN_RETIRO", "EN_CAMINO", "DEMORADO_RETIRO", "DEMORADO_ENTREGA"].includes(estadoId);
  ...
  return (
    <div className="m-6 space-y-6 max-w-2xl">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <StatusBadge
          estado={toEstadoKey(envio.estadoId)}
          label={estadoLabel(envio.estadoId)}
        />
      </div>
    ...
    : "text-slate-400"
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-5 space-y-4">
        ...
        <InfoRow
          ...
          label="Paquetes"
        >
          {envio.paquetes?.length ?? 0} bulto
          {envio.paquetes?.length !== 1 ? "s" : ""}
        </InfoRow>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {puedeCancelar && (
            <button
              type="button"
              onClick={() =>
                accion(() => cancelarPorComisionista(id), "cancelar")
              }
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-5">
          ...
          {calificacion ? (
            ...
            <p className="text-sm text-slate-400 italic">
              El cliente aún no calificó este envío.
            </p>
          )}
        </div>
      )}
