...
useEffect(() => {
    ...
    if (provincias.length === 0 || vehiculos.length === 0) return;

    const inter = Array.isArray(initial.intermedias) ? initial.intermedias : [];

    // Seteamos los refs ANTES de setear el form, para que los efectos
    // de origen/destino no disparen un reset de localidadId
    prevOrigenProvRef.current = initial.origen?.provinciaId || "";
    prevDestinoProvRef.current = initial.destino?.provinciaId || "";
    ...
    // Cargar todas las localidades en paralelo desde acá
    (async () => {
        const [locsOrigen, locsDestino] = await Promise.all([
            initial.origen?.provinciaId
                ? getLocalidadesByProvincia(initial.origen.provinciaId).catch(() => [])
                : Promise.resolve([]),
            initial.destino?.provinciaId
                ? getLocalidadesByProvincia(initial.destino.provinciaId).catch(() => [])
                : Promise.resolve([]),
        ]);
...
}, [open, initial, provincias.length, vehiculos.length]);


    // ORIGEN: solo para cuando el usuario cambia la provincia manualmente
useEffect(() => {
    if (!open) return;
    ...
    // Si coincide con el ref, es hidratación → ya cargamos arriba, no hacer nada
    if (pid === prevOrigenProvRef.current) return;

    // El usuario cambió la provincia manualmente → reset localidad + fetch
    ...
    // DESTINO: solo para cuando el usuario cambia la provincia manualmente
useEffect(() => {
    if (!open) return;
    ...
    // Si coincide con el ref, es hidratación → ya cargamos arriba, no hacer nada
    if (pid === prevDestinoProvRef.current) return;
        ...
        <div className="fixed inset-0 z-50">
            ...
            <div className="absolute left-1/2 top-1/2 w-[95%] max-w-3xl -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white shadow-xl border max-h-[90vh] overflow-hidden flex flex-col">
                ...
                <form onSubmit={submit} className="p-5 space-y-5 overflow-y-auto flex-1">
                    ...
                    <div className="rounded-xl border p-4 space-y-2">
                        ...
                        >
                            ...
                            {vehiculos.map((v) => {
                                ...
                                const label = parts.join(" · ") || String(v._id);
                            ...
                            {DIAS.map((d) => {
                                ...
                                return (
                                    <button
                                        ...
                                        type="button"
                                        onClick={() => toggleDia(d)}
                                        className={`px-3 py-1.5 rounded-full border text-sm font-semibold ${active
                    ...
                    {/* Descuento por cantidad de bultos */}
                    <div className="rounded-xl border p-4 space-y-3">
                        <div>
